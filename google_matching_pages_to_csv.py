...
def main(source_url, no_results, name, country_code, host_language):
    ...
    for i in range(0, pages):
        ...
        with open(fname, 'r') as f:
            ...
            # new 2024+ Lens "Exact matches" layout
            if not results:
                candidates = soup.find_all('div', {'class': 'MjjYud'})
                # filter out tracking/script placeholders without a result link
            ...
            for result in results:
                ...
                # --- new Lens layout ---
                new_link = result.find('a', {'class': 'ngTNl'})
                if new_link is not None:
                    try:
                        search_result['url'] = new_link.get('href')
                    ...
                    try:
                        title_el = result.find('div', {'class': 'ZhosBf'})
                        search_result['title'] = title_el.get_text() if title_el else new_link.get_text()
                    except Exception:
                        search_result['title'] = None
                    try:
                        search_result['description'] = result.get_text(separator=' | ')
                        search_result['text'] = search_result['description']
                    except Exception:
                ...
                # dedupe identical urls (Lens page contains repeats)
                if search_result.get('url') and any(
                        r.get('url') == search_result['url'] for r in search_results):
                    continue
    ...
    if not search_results:
        ...
        search_results = [{'rank': 0, 'possible_related_search': possible_related_search,
                           'url': None, 'domain': None, 'country_code_tld': None,
                           'title': None, 'description': None, 'text': None,
                           'thumbnail_res': None, 'date': None, 'thumbnail_url': None}]
