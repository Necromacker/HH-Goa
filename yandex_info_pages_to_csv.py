...
def main(source_url, no_results, name):
    ...
    with open(fname, 'r') as f:
        ...
        results = soup.find_all(class_='CbirSites-Item')
        ...
        for result in results:
            ...
            try:
                # small thumbnails: either <img class="Thumb-Image"> or
                # <div class="Thumb-Image" style="background-image: url(...)">
                thumb = result.find(class_='Thumb-Image')
                thumb_url = None
                if thumb is not None:
                    if thumb.name == 'img' and thumb.has_attr('src'):
                        src = thumb['src']
                        thumb_url = src if src.startswith('http') else 'https:' + src
                    elif thumb.has_attr('style'):
                        import re as _re
                        m = _re.search(r'url\(["\']?(//[^"\')]+)', thumb['style'])
                        if m:
    ...
    if not search_results:
        ...
        search_results = [{'rank': 0, 'url': None, 'domain': None,
                           ...
                           'description': None, 'thumbnail_res': None,
                           'thumbnail_url_original': None, 'thumbnail_url': None}]
